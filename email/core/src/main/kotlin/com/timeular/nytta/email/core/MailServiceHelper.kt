...
open class MailServiceHelper @JvmOverloads constructor(
    ...
    fun modifyTemplateContextForOverride(
        mailContext: Map<String, Any>,
        mailCfgBuilder: MailConfig.Builder
    ): Map<String, Any> =
        if (isOverrideEnabled) {
            val resultMap = HashMap(mailContext)
            resultMap["overrideAddress"] = overrideAddress.toString()
            resultMap["originalAddressTo"] = mailCfgBuilder.to.joinToString(",")
            resultMap["originalAddressCC"] = mailCfgBuilder.cc.joinToString(",")
            resultMap["originalAddressBcc"] = mailCfgBuilder.bcc.joinToString(",")
